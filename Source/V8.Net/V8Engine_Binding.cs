namespace V8.Net
{
    ...
    public class TypeLibrary<T> where T : class
    {
        ...
        public T Set(Type[] types, T value)
        {
            ...
            return Set(types, value, 0);
        }

        private T Set(Type[] types, T value, int depth)
        {
            ...
            if (SubTypes.TryGetValue(type, out typeLibrary))
                return typeLibrary.Set(types, value, depth + 1);
        ...
        private T Get(Type[] types, int depth)
        {
            if (depth >= types.Length) return Object;
            var type = types[depth];
            TypeLibrary<T> typeLibrary;
            return !SubTypes.TryGetValue(type, out typeLibrary)
                ? null
                : typeLibrary.Get(types, depth + 1);
        }
        ...
        IEnumerable<TypeLibrary<T>> _GetItems()
        {
            ...
            foreach (var item in SubTypes.SelectMany(kv => kv.Value._GetItems()))
                yield return item;
        }
    }
    ...
    public struct ArgInfo
    {
        ...
        public ArgInfo(InternalHandle handle)
            : this(handle, null, null)
        {
        }

        public ArgInfo(InternalHandle handle, ParameterInfo paramInfo)
            : this(handle, paramInfo, null)
        {
        }
        ...
        public ArgInfo(InternalHandle handle, ParameterInfo paramInfo, Type expectedType)
        {
            ...
            if (handle.ClrTypeId >= 0) // (must be an object type with ID <= -2)
            {
                ...
                Type = TypeId >= 0 ? handle.Engine.RegisteredTypesInternal[TypeId] : null; // (this will return 'null' if the index is invalid)
            }
            else
            {
        ...
        /// </summary>
        public static ArgInfo[] GetArguments(InternalHandle[] handles)
        {
            return GetArguments(handles, 0, null);
        }
        ...
        /// </summary>
        public static ArgInfo[] GetTypes(InternalHandle[] handles)
        {
            return GetTypes(handles, 0, null);
        }
        ...
        {
            var infoList = argInfoList as IList<ArgInfo> ?? argInfoList.ToList();

            int count = infoList.Count(), i = 0;
            var types = new Type[count];
            var enumerator = infoList.GetEnumerator();
        ...
        {
            var handles = new InternalHandle[argInfoArgs.Length];
            for (var i = 0; i < argInfoArgs.Length; i++)
    ...
    public sealed class TypeBinder
    {
        ...
        /// </summary>
        public bool Recursive { get { return RecursiveInternal ?? (BaseTypeBinder != null && BaseTypeBinder.Recursive); } }
        internal bool? RecursiveInternal;

        /// <summary>
        /// Default member attributes for members that don't have the 'ScriptMember' attribute.
        /// </summary>
        public ScriptMemberSecurity DefaultMemberSecurity
        {
            get
            {
                return DefaultMemberSecurityInternal ??
                       (BaseTypeBinder != null
                           ? BaseTypeBinder.DefaultMemberSecurity
                           : Engine.DefaultMemberBindingSecurity);
            }
        }

        internal ScriptMemberSecurity? DefaultMemberSecurityInternal;
        ...
        /// </summary>
        public ScriptObject ScriptObjectAttribute
        {
            get { return m_scriptObjectAttributeInternal; }
        }

        private readonly ScriptObject m_scriptObjectAttributeInternal;

#if !(V1_1 || V2 || V3 || V3_5)
        [DebuggerDisplay("{MemberName}x{TotalImmediateMembers}")]
#endif
        internal class MemberDetailsInternal
        {
            public readonly TypeBinder TypeBinder;
            public MemberDetailsInternal(TypeBinder owner) { TypeBinder = owner; }

            public MemberDetailsInternal BaseDetails; // (if set, this is these are the inherited members of the same name represented by this member).
            ...
            public uint TotalConstructedMembers; // (if > 1 then this member is overloaded)
            ...
            public ScriptMemberSecurity InheritedMemberSecurity { get { return MemberSecurity ?? (BaseDetails != null ? BaseDetails.InheritedMemberSecurity : ScriptMemberSecurity.NoAcccess); } }
        ...
        private MemberDetailsInternal m_constructors;

        internal readonly Dictionary<string, MemberDetailsInternal> MembersInternal = new Dictionary<string, MemberDetailsInternal>();

        public IEnumerable<MemberInfo> Members
        {
            get { return from m in MembersInternal.Values from mi in m.Members select mi; }
        }

        private IEnumerable<MemberDetailsInternal> _FieldDetails(BindingMode bindingMode)
        {
            return from kv in MembersInternal
                where
                    (bindingMode == BindingMode.None || kv.Value.BindingMode == bindingMode) &&
                    kv.Value.MemberType == MemberTypes.Field
                select kv.Value;
        }

        private IEnumerable<MemberDetailsInternal> _PropertyDetails(BindingMode bindingMode)
        {
            return from kv in MembersInternal
                where
                    (bindingMode == BindingMode.None || kv.Value.BindingMode == bindingMode) &&
                    kv.Value.MemberType == MemberTypes.Property
                select kv.Value;
        }

        private IEnumerable<MemberDetailsInternal> _MethodDetails(BindingMode bindingMode)
        {
            return from kv in MembersInternal
                where
                    (bindingMode == BindingMode.None || kv.Value.BindingMode == bindingMode) &&
                    kv.Value.MemberType == MemberTypes.Method
                select kv.Value;
        }
        ...
        internal TypeBinder(V8Engine engine, Type type)
            : this(engine, type, null, null, null)
        {
        }
        ...
        internal TypeBinder(V8Engine engine, Type type, string className, bool? recursive, ScriptMemberSecurity? defaultMemberSecurity)
        {
            ...
            Engine = engine;
            BoundType = type;
            TypeId = Engine.RegisteredTypesInternal.Add(type); // (this is done here to make sure the type is created and accessible to prevent cyclical calls)
            RecursiveInternal = recursive;
            DefaultMemberSecurityInternal = defaultMemberSecurity;
            m_scriptObjectAttributeInternal = (from a in type.GetCustomAttributes(true) where a is ScriptObject select (ScriptObject)a).FirstOrDefault();

            if (DefaultMemberSecurityInternal == null && m_scriptObjectAttributeInternal != null)
                DefaultMemberSecurityInternal = m_scriptObjectAttributeInternal.Security;

            if (className.IsNullOrWhiteSpace())
            {
                if (m_scriptObjectAttributeInternal != null)
                    ClassName = m_scriptObjectAttributeInternal.TypeName;
            ...
            Engine.BindersInternal[type] = this;
        ...
        void _BindInstanceMembers()
        {
            ...
            for (mi = 0; mi < members.Length; mi++)
            {
                ...
                var scriptMemberAttrib = (from a in member.GetCustomAttributes(true) where a is ScriptMember select (ScriptMember)a).LastOrDefault();
                ...
                _CreateMemberDetails(memberName, memberSecurity, member,
                    s => MembersInternal.GetValueOrDefault(s),
                    md => MembersInternal[md.MemberName] = md);
            }
            ...
            foreach (var tb in BaseBinders) // (this will return the inherited base types in order from the immediate base upwards, which is really important when linking '_MemberDetails' instances)
            {
                ...
                var baseInstanceMembers = from md in tb.MembersInternal.Values where md.BindingMode == BindingMode.Instance select md;
                foreach (var baseInstanceMemberDetails in baseInstanceMembers)
                {
                    var md = MembersInternal.GetValueOrDefault(baseInstanceMemberDetails.MemberName);
                    if (md == null)
                        MembersInternal[baseInstanceMemberDetails.MemberName] = baseInstanceMemberDetails; // (adopt the member into this instance as well for fast lookup)
                    else
        ...
        internal MemberDetailsInternal _CreateMemberDetails(string memberName, ScriptMemberSecurity? memberSecurity, MemberInfo memberInfo,
            Func<string, MemberDetailsInternal> getExisting, // (called to check if there's an existing member by the same name)
            Action<MemberDetailsInternal> set) // (called when no member exists and needs to be added [i.e. no existing member details were updated])
        {
            if (memberName.IsNullOrWhiteSpace())
                memberName = memberInfo.Name;

            MemberDetailsInternal memberDetails = null;
            ...
            if (memberInfo.MemberType == MemberTypes.Field)
            {
                ...
                memberDetails = new MemberDetailsInternal(this)
                {
            ...
            else if (memberInfo.MemberType == MemberTypes.Method)
            {
                ...
                if (!methodInfo.IsSpecialName)
                {
                    ...
                    memberDetails = existingMemberDetails ?? new MemberDetailsInternal(this)
                   {
            ...
            else if (memberInfo.MemberType == MemberTypes.Constructor)
            {
                ...
                if (m_constructors != null)
                {
                    ...
                    memberDetails = m_constructors = new MemberDetailsInternal(this)
                    {
        ...
        internal bool _GetBindingForDataMember(MemberDetailsInternal memberDetails, out V8NativeObjectPropertyGetter getter, out V8NativeObjectPropertySetter setter)
        {
        ...
        internal bool _GetBindingForField(MemberDetailsInternal memberDetails, out V8NativeObjectPropertyGetter getter, out V8NativeObjectPropertySetter setter)
        {
            ...
            else if (RecursiveInternal ?? false)
            {
        ...
        V8NativeObjectPropertySetter _CreateSetAccessor(MemberDetailsInternal memberDetails, FieldInfo fieldInfo)
        {
        ...
        V8NativeObjectPropertyGetter _CreateGetAccessor<T>(MemberDetailsInternal memberDetails, FieldInfo fieldInfo)
        {
                ...
                return (InternalHandle _this, string propertyName) =>
                {
                    ...
                    try
                    {
                        return _this.IsBinder
                            ? Engine.CreateValue((T)fieldInfo.GetValue(_this.BoundObject))
                            : Engine.CreateError("The ObjectBinder is missing for property '" + propertyName + "' (" + fieldInfo.Name + ").", JSValueType.ExecutionError);
                    }
                    catch (Exception ex) { return Engine.CreateValue(ex.GetFullErrorMessage()); }
                };
        ...
        V8NativeObjectPropertyGetter _CreateObjectGetAccessor(MemberDetailsInternal memberDetails, FieldInfo fieldInfo)
        {
            var isSystemType = BoundType.Namespace == "System";
            ...
            if (isSystemType)
                return (InternalHandle _this, string propertyName) =>
                {
                    ...
                    {
                        return _this.IsBinder
                            ? Engine.CreateValue(fieldInfo.GetValue(_this.BoundObject), RecursiveInternal)
                            : Engine.CreateError("The ObjectBinder is missing for property '" + propertyName + "' (" + fieldInfo.Name + ").", JSValueType.ExecutionError);
                    }
                    catch (Exception ex) { return Engine.CreateValue(ex.GetFullErrorMessage()); }
                };
            ...
            if (isSystemType)
                return (InternalHandle _this, string propertyName) =>
                {
                    ...
                    if (propertyInfo.CanRead)
                    {
                        ...
                        {
                            return _this.IsBinder
                                ? Engine.CreateValue((T)propertyInfo.GetValue(_this.BoundObject, null))
            ...
            if (isSystemType)
                return (InternalHandle _this, string propertyName) =>
                {
                    if (propertyInfo.CanRead)
                    {
                        ...
                        {
                            return _this.IsBinder
                                ? Engine.CreateValue(propertyInfo.GetValue(_this.BoundObject, null), RecursiveInternal)
                                : Engine.CreateError("The ObjectBinder is missing for property '" + propertyName + "' (" + propertyInfo.Name + ").", JSValueType.ExecutionError);
                        }
                        catch (Exception ex) { return Engine.CreateValue(ex.GetFullErrorMessage()); }
                    }
        ...
        public static string GetMethodSignatureAsText(MethodInfo methodInfo)
        {
            return GetMethodSignatureAsText(methodInfo, -1);
        }
            ...
            out TypeLibrary<MemberInfo> constructedMembers, ref ParameterInfo[] expectedParameters)
        {
            ...
            var typeArgs = new InternalHandle[args[0].ArrayLength];

            for (var i = 0; i < args[0].ArrayLength; i++)
            ...
            var genericArgInfos = ArgInfo.GetTypes(typeArgs, 0, expectedGenericTypes);
            ...
            if (constructedMembers == null)
            {
                ...
                for (var i = 0; i < genericMethodInfos.Length; i++)
                {
                    var constructedMethod = genericMethodInfos[i].MakeGenericMethod(genericSystemTypes);
        ...
        InternalHandle _InvokeMethod(MemberDetailsInternal memberDetails, MethodInfo soloMethod, object[] convertedArguments, ref InternalHandle _this,
            ArgInfo[] argInfos, TypeLibrary<MemberInfo> constructedMembers)
        {
            if (soloMethod != null)
            {
                ...
                return soloMethod.ReturnType == typeof(void) ? InternalHandle.Empty : Engine.CreateValue(result, RecursiveInternal);
            }
            else // ... more than one method exists (overloads) ..
            {
                ...
                return methodInfo.ReturnType == typeof(void) ? InternalHandle.Empty : Engine.CreateValue(result, RecursiveInternal);
            }
        }
        ...
        internal bool _GetBindingForMethod(MemberDetailsInternal memberDetails, out V8Function func)
        {
            return _GetBindingForMethod(memberDetails, out func, null);
        }
        ...
        /// <param name="genericTarget">Allows binding a specific handle to the 'this' of a callback (for static bindings).</param>
        /// <param name="genericInstanceTargetUpdater">Allows binding a specific instance to the 'this' of a callback (for static bindings).</param>
        internal bool _GetBindingForMethod(MemberDetailsInternal memberDetails, out V8Function func, string className)
        {
            func = null;

            if (memberDetails == null)
                throw new ArgumentNullException("memberDetails");

            if (TypeTemplate == null)
                throw new InvalidOperationException("'TypeTemplate' is null.");

            if (BoundType == null)
                throw new InvalidOperationException("'BoundType' is null.");
            ...
            func = funcTemplate.GetFunctionObject<V8Function>((V8Engine engine, bool isConstructCall, InternalHandle _this, InternalHandle[] args) =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    return Engine.CreateError(msg + argError + ex.GetFullErrorMessage(), JSValueType.ExecutionError);
                }
            });
                ...
                if (isConstructCall)
                    try
                    {
                        ...
                        else
                        {
                            ...
                            handle = Engine.CreateBinding(Activator.CreateInstance(BoundType, convertedArguments), null, RecursiveInternal, DefaultMemberSecurityInternal, false);
                    ...
                    {
                        handle = Engine.CreateError("Failed to invoke constructor: " + ex.GetFullErrorMessage(), JSValueType.ExecutionError);
                    }
                else
                    try
                    {
                    ...
                    {
                        handle = Engine.CreateError("Failed to strongly type the specified value '" + args[0].Value + "': " + ex.GetFullErrorMessage(), JSValueType.ExecutionError);
                    }
        ...
        public T CreateObject<T, TInstanceType>(TInstanceType obj)
            where T : ObjectBinder, new()
            where TInstanceType : class
        {
            return CreateObject<T, TInstanceType>(obj, true);
        }
        ...
        internal MemberDetailsInternal _GetMemberDetails(MemberInfo member)
        {
            var members = from md in MembersInternal.Values where md.ImmediateMembers.Items.Any(i => i.Object.MetadataToken == member.MetadataToken && i.Object.Module == member.Module) select md;
        ...
        {
            var memberDetails = (from kv in MembersInternal where kv.Key == memberName select kv.Value).FirstOrDefault();
    ...
    public class ObjectBinder : V8ManagedObject
    {
        ...
        public override InternalHandle IndexedPropertyGetter(int index)
        {
            if (TypeBinder.Indexer != null && TypeBinder.Indexer.CanRead)
                return Engine.CreateValue(TypeBinder.Indexer.GetValue(Object, new object[] { index }), TypeBinder.RecursiveInternal);
            return InternalHandle.Empty;
        }
        ...
        {
            var memberDetails = TypeBinder.MembersInternal.GetValueOrDefault(propertyName);
            if (memberDetails == null || !memberDetails.Accessible)
                return InternalHandle.Empty;
        ...
        public override InternalHandle NamedPropertySetter(ref string propertyName, InternalHandle value)
        {
        ...
        {
            var memberDetails = TypeBinder.MembersInternal.GetValueOrDefault(propertyName);
            if (memberDetails == null || memberDetails.HasSecurityFlags(ScriptMemberSecurity.NoAcccess))
                return InternalHandle.Empty;
        ...
        {
            return Engine.CreateValue(TypeBinder.MembersInternal.Keys);
        }
    ...
    public partial class V8Engine
    {
        ...
        /// </summary>
        internal readonly Dictionary<Type, TypeBinder> BindersInternal = new Dictionary<Type, TypeBinder>();

        public IEnumerable<TypeBinder> Binders
        {
            get { return BindersInternal.Values; }
        }
        ...
        /// </summary>
        internal IndexedObjectList<Type> RegisteredTypesInternal = new IndexedObjectList<Type>();
        ...
        public TypeBinder RegisterType(Type type)
        {
            return RegisterType(type, null, null, null);
        }
        ...
        public TypeBinder RegisterType(Type type, string className, bool? recursive, ScriptMemberSecurity? memberSecurity)
        {
            ...
            if (recursive != null)
                binder.RecursiveInternal = recursive.Value;

            if (memberSecurity != null)
                binder.DefaultMemberSecurityInternal = memberSecurity;
        ...
        /// <param name="recursive">When an object is bound, only the object instance itself is bound (and not any reference members). If true, then nested object references are included.</param>
        /// <param name="memberSecurity">Default member attributes for members that don't have the 'ScriptMember' attribute.</param>
        public TypeBinder RegisterType<T>()
        {
            return RegisterType<T>(null, null, null);
        }
        ...
        /// <param name="memberSecurity">Default member attributes for members that don't have the 'ScriptMember' attribute.</param>
        public InternalHandle CreateBinding(Type type)
        {
            return CreateBinding(type, null, null, null);
        }
        ...
        /// If true, then nested object references are included.</param>
        /// <param name="memberSecurity">Default member attributes for members that don't have the 'ScriptMember' attribute.</param>
        public InternalHandle CreateBinding<T>()
        {
            return CreateBinding<T>(null, null, null);
        }
        ...
        /// <param name="memberSecurity">Default member attributes for members that don't have the 'ScriptMember' attribute.</param>
        /// <param name="initializeBinder">If true (default) then then 'IV8NativeObject.Initialize()' is called on the created object before returning.</param>
        public InternalHandle CreateBinding(object obj)
        {
            return CreateBinding(obj, null, null, null, true);
        }
