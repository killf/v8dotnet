namespace V8.Net
{
    ...
    public unsafe class FunctionTemplate : TemplateBase<IV8Function>, IFinalizable
    {
        ...
        internal NativeFunctionTemplateProxy* NativeFunctionTemplateProxy;
        ...
        /// </summary>
        readonly Dictionary<Type, int> m_functionsByType = new Dictionary<Type, int>();
        ...
        public void Dispose()
        {
            ...
            {
                V8NetProxy.DeleteFunctionTemplateProxy(NativeFunctionTemplateProxy); // (delete the corresponding native object as well; WARNING: This is done on the GC thread!)
                NativeFunctionTemplateProxy = null;
        ...
        internal void _Initialize(V8Engine v8EngineProxy, NativeFunctionTemplateProxy* nativeFunctionTemplateProxy)
        {
            ...
            NativeFunctionTemplateProxy = nativeFunctionTemplateProxy;

            InstanceTemplate = Engine.CreateObjectTemplate<ObjectTemplate>();
            InstanceTemplate.Parent = this;
            InstanceTemplate._Initialize(Engine, V8NetProxy.GetFunctionInstanceTemplateProxy(NativeFunctionTemplateProxy));

            PrototypeTemplate = Engine.CreateObjectTemplate<ObjectTemplate>();
            PrototypeTemplate.Parent = this;
            PrototypeTemplate._Initialize(Engine, (NativeObjectTemplateProxy*)V8NetProxy.GetFunctionPrototypeTemplateProxy(NativeFunctionTemplateProxy));
        ...
        HandleProxy* _CallBack(Int32 managedObjectId, bool isConstructCall, HandleProxy* _this, HandleProxy** args, Int32 argCount)
        {
            var functions = from f in
                                (from t in m_functionsByType.Keys.ToArray() // (need to convert this to an array in case the callbacks modify the dictionary!)
                                 select Engine._GetObjectWeakReference(m_functionsByType[t]))
        ...
        internal static HandleProxy* _CallBack(Int32 managedObjectId, bool isConstructCall, HandleProxy* _this, HandleProxy** args, Int32 argCount, params JSFunction[] functions)
        {
            ...
            using (var hThis = new InternalHandle(_this, false))
            {
                var engine = hThis.Engine;

                // ... wrap the arguments ...

                var argsInternal = new InternalHandle[argCount];
                int i;

                for (i = 0; i < argCount; i++)
                    argsInternal[i]._Set(args[i], false); // (since these will be disposed immediately after, the "first" flag is not required [this also prevents it from getting passed on])
                ...
                try
                {
                    ...
                    {
                        result = callback(engine, isConstructCall, hThis, argsInternal);
                ...
                {
                    for (i = 0; i < argsInternal.Length; i++)
                        if (argsInternal[i] != result)
                            argsInternal[i].Dispose();
                }
        ...
        public T GetFunctionObject<T>() where T : V8Function, new()
        {
            return GetFunctionObject<T>(null);
        }
        ...
        public T GetFunctionObject<T>(JSFunction callback) where T : V8Function, new()
        {
            ...
            if (m_functionsByType.TryGetValue(typeof(T), out funcId))
            {
                var weakRef = Engine._GetObjectWeakReference(funcId);
            ...
            InternalHandle hNativeFunc = V8NetProxy.GetFunction(NativeFunctionTemplateProxy);

            // ... create a managed wrapper for the V8 "Function" object (note: functions inherit the native V8 "Object" type) ...

            func = Engine._GetObject<T>(this, hNativeFunc.PassOn(), true, false, true); // (note: this will "connect" the native object [hNativeFunc] to a new managed V8Function wrapper, and set the prototype!)
        ...
        {
            HandleProxy** argsInternal = null;

            if (args.Length > 0)
            {
                argsInternal = (HandleProxy**)Utilities.AllocPointerArray(args.Length);
                for (var i = 0; i < args.Length; i++)
                    argsInternal[i] = args[i];
            }

            try
            {
                return V8NetProxy.CreateFunctionInstance(NativeFunctionTemplateProxy, -1, args.Length, argsInternal);
            }
            finally
            {
                Utilities.FreeNativeMemory((IntPtr)argsInternal);
            }
        }
            ...
            where T : V8ManagedObject, new()
        {
            ...
            T obj = Engine._CreateManagedObject<T>(this, null);
        ...
        {
            var callbackTypes = m_functionsByType.Keys.ToArray();
            foreach (var t in callbackTypes.Where(t => m_functionsByType[t] == objectId))
                m_functionsByType[t] = -1;
        }
        ...
        {
            SetProperty(name, value, V8PropertyAttributes.Undefined);
        }
        ...
        public void SetProperty(string name, InternalHandle value, V8PropertyAttributes attributes)
        {
            try
            {
                ...
                V8NetProxy.SetFunctionTemplateProperty(NativeFunctionTemplateProxy, name, value, attributes);
            }
            finally
            {
