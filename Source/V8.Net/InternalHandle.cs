namespace V8.Net
{
        ...
        IDynamicMetaObjectProvider
    {
        ...
        private bool m_first; // (this is true if this is the FIRST handle to wrap the proxy [first handles may become automatically disposed internally if another handle is not created from it])
        ...
        /// </summary>
        internal InternalHandle(HandleProxy* hp) : this(hp, true)
        {
        }
        ...
        public InternalHandle Set(InternalHandle handle)
        {
            if (handle.m_first)
            {
                var h = _Set(handle.HandleProxyInternal);
            ...
            else return _Set(handle.HandleProxyInternal);
        }
        ...
        internal InternalHandle _Set(HandleProxy* hp)
        {
            return _Set(hp, true);
        }
        ...
        internal InternalHandle _Set(HandleProxy* hp, bool checkIfFirst)
        {
            ...
            if (HandleProxyInternal != null)
            {
                ...
                if (handleId >= engine.HandleProxiesInternal.Length)
                {
                    HandleProxy*[] handleProxies = new HandleProxy*[(100 + handleId) * 2];
                    for (var i = 0; i < engine.HandleProxiesInternal.Length; i++)
                        handleProxies[i] = engine.HandleProxiesInternal[i];
                    engine.HandleProxiesInternal = handleProxies;
                }

                engine.HandleProxiesInternal[handleId] = HandleProxyInternal;

                if (checkIfFirst)
                    m_first = (HandleProxyInternal->ManagedReferenceCount == 0);
        ...
        {
            return handleProxy != null ? new InternalHandle(handleProxy) : Empty;
        }
        ...
        {
            return handle.ToString(CultureInfo.CurrentCulture);
        }
        ...
        {
            var h = this;
            m_first = false; // ("first" is normally only true if the system created handle is not set to another handle)
            return h;
        }
        ...
        /// </summary>
        public Int32 ClrTypeId { get { return HandleProxyInternal != null ? HandleProxyInternal->_CLRTypeID : -1; } }
        ...
        private const string NotAnObjectErrorMessage = "The handle does not represent a JavaScript object.";
        ...
        public bool SetProperty(string name, InternalHandle value)
        {
        ...
        public bool SetProperty(string name, InternalHandle value, V8PropertyAttributes attributes)
        {
            try
            {
                ...
                if (!IsObjectType)
                    throw new InvalidOperationException(NotAnObjectErrorMessage);
        ...
        public bool SetProperty(Int32 index, InternalHandle value)
        {
            try
            {
                if (!IsObjectType) throw new InvalidOperationException(NotAnObjectErrorMessage);
        ...
        public bool SetProperty(string name, object obj, string className, bool? recursive, ScriptMemberSecurity? memberSecurity)
        {
            ...
            var handleBased = obj as IHandleBased;
            if (handleBased != null)
                return SetProperty(name, handleBased.AsInternalHandle, (V8PropertyAttributes)(memberSecurity ?? ScriptMemberSecurity.ReadWrite));
        ...
        /// <param name="memberSecurity">For object instances, these are default flags that describe JavaScript properties for all object instance members that
        /// don't have any 'ScriptMember' attribute.  The flags should be 'OR'd together as needed.</param>
        public bool SetProperty(Type type)
        {
        ...
        {
            if (functionName.IsNullOrWhiteSpace()) throw new ArgumentNullException("functionName");
        ...
        public InternalHandle StaticCall(string functionName, params InternalHandle[] args)
        {
            ...
            return _Call(functionName, Empty, args);
        }
    ...
    {
        private readonly Dictionary<Int32, Dictionary<string, Delegate>> m_accessors = new Dictionary<Int32, Dictionary<string, Delegate>>();
        ...
        internal T _StoreAccessor<T>(Int32 id, string propertyName, T d) where T : class
        {
            ...
            if (!m_accessors.TryGetValue(id, out delegates))
                m_accessors[id] = delegates = new Dictionary<string, Delegate>();
        ...
        {
            Dictionary<string, Delegate> delegates;
            return m_accessors.TryGetValue(id, out delegates) && delegates.Count > 0;
        }
