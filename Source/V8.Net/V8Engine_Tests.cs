namespace V8.Net
{
    ...
    public unsafe partial class V8Engine
    {
        ...
        private void _ThrowMarshalTestError(string structName, string fieldName, byte offset, byte[] goodData, byte* badValues)
        {
            _ThrowMarshalTestError(structName, fieldName, offset, goodData, badValues, 0);
        }

        private void _ThrowMarshalTestError(string structName, string fieldName, byte offset, byte[] goodData, byte* badValues, int badValLen)
        {
            var details = "";
            ...
            if (badValues == null)
                throw new Exception(
                    string.Format(
                        "The field value for '{0}->{1}' (offset {2}) on the native side proxy struct does not align with the managed side struct." +
                        details,
        ...
        private double _GetMarshalTestDoubleValue(byte ofs, out byte[] data)
        {
        ...
        private Int64 _GetMarshalTestPTRValue(byte ofs, out byte[] data)
        {
            IntPtr mem = Marshal.AllocCoTaskMem(8);
